...
2. **調査とまとめの作成**:
   ...
   - **情報の最新性の確認と検索の徹底**:
     - 古い知識やAIの事前知識だけで答えず、変化しうる情報は検索してから記事にしてください。
     - 情報が最新性を必要とするか判断してください。
     - ニュース、企業情報、製品仕様、料金、法律・制度、AIモデル情報、サービス内容、イベント情報、**スポーツ大会の登録メンバーや出場選手といった流動的な情報**など、変化する可能性がある内容は必ず検索してください。
     - 特に人物や組織の選出・所属状況、ステータスについては、最新の公式情報や信頼できるメディア情報を必ず検索して照合してください。
...
- **読みやすさ**: `p`, `ul`, `ol`, `table`, `blockquote` をバランスよく使用し、重要な箇所は太字（`<strong>`）にします。**HTMLファイル内ではMarkdownの太字記述（`**...**`）は機能せず、そのまま表示されてしまうため、必ず `<strong>` タグを使用してください。**
