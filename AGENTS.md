...
記事は「信頼できる解説メモ」としての読み心地を最優先とします。以下のルールを守ってください。

- **平易で簡潔な文を書く**: 一文は60〜80文字を目安に区切り、主語と述語の距離を近くに保ちます。修飾語の連鎖や多重の入れ子構造は避けてください。
- **AI感のある誇張表現を使わない**: 以下のような表現は禁止します。
  - 「〜が革命的に変わります」「圧倒的な〜」「驚くべき〜」「画期的な〜」「まさに〜と言えるでしょう」
  - 「〜を完全に網羅」「徹底解説」「〜のすべて」（タイトル・本文ともに）
  - 「必見です」「見逃せません」「〜しないと損」
  - 「いかがでしたか？」「〜ではないでしょうか」（読者への不要な問いかけ）
  - 感嘆符（！）の多用（見出しでは原則使わない。本文中も最小限に）
- **事実と意見を分ける**: 客観的な事実はそのまま述べ、筆者の評価や推測を加える場合は「〜と考えられます」「〜の傾向があります」など、根拠の度合いが伝わる表現を使います。
- **具体的に書く**: 「非常に高速」「大幅に向上」のような曖昧な修飾は避け、可能な限り数値・比較対象・条件を添えます（例: 「従来比で約30%高速」）。
- **体言止めを多用しない**: 見出しでは許容しますが、本文中の文末を体言止めで連続させないでください。文末は「〜です」「〜ます」「〜します」で自然に結びます。
- **語尾の単調な繰り返しを避ける**: 「〜です。〜です。〜です。」のように同じ語尾が3回以上続く場合は、語順の変更や文の統合で変化をつけてください。
- **接続詞を適切に使う**: 「また」「さらに」「なお」を過度に連続させず、文の論理関係に合った接続詞（「一方」「ただし」「具体的には」など）を選びます。
- **漢字とひらがなのバランス**: 補助動詞（「〜して頂く」→「〜していただく」）、形式名詞（「〜する事」→「〜すること」）、接続助詞的な用法（「〜の為」→「〜のため」）はひらがなで書きます。漢字が連続して硬い印象になる場合は適宜ひらがなに開いてください。
